...
`.mental/` is the user's private, gitignored continuity log for one repository.
It preserves only what git cannot explain: current focus, consequential decisions
and rationale, durable repository-specific facts, and the next exact action.
...
If `.mental/` is absent, first run
`git check-ignore -q -- .mental/probe`. Create the minimal bundle only when that
check succeeds. When it fails, continue the requested work without `.mental/`
and tell the user to run `npx balakit doctor`. Never modify global git
configuration or the repository's `.gitignore`.
...
`.mental/` is optional, per-repository, and machine-local. Never stage, commit,
publish, or expose its contents in PRs, issues, release notes, code comments, or
external messages. Never store secrets. Cross-repository personal knowledge
belongs in the agent's own memory system.
