...
import { recordInstall, readManifest, projectManifestPath } from "./manifest.mjs";
...
export function buildInstallPlan({
  ...
  if (reconcile) {
    ...
    names = [...new Set([...proj.rules, ...names])];
  }
...
export function formatPlanReview(plan, { dryRun = false } = {}) {
  ...
  if (plan.skills.length) {
    ...
    lines.push(`Skills (project): ${plan.skills.map(label).join(", ")}`);
  }
...
export async function runInstallPlan(plan, { dryRun = false, yes = false, review = true } = {}) {
  ...
  if (!runSkillBatch(plan.skills, "project")) skillsFailed = true;
